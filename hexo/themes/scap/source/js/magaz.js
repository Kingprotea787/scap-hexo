  ...
  async function loadPDF(mode) {
      ...
      try {
        ...
        if (mode === 'flip') {
          ...
          currentPage = 1;
          updatePageIndicator();
          // —— traditional 模式（已按你的要求改成：单页 90vw，双页 45vw；高度按 PDF 宽高比自适应）——
          } else {
            traditionalContainer.innerHTML = '';

            const isDouble = spreadSwitch.checked; // true=双页，false=单页
            traditionalContainer.classList.toggle('single-page', !isDouble);

            // 用 grid 居中排版，每个 canvas 宽度用 vw 控制
            traditionalContainer.style.display = 'grid';
            traditionalContainer.style.gridAutoRows = 'auto';
            traditionalContainer.style.justifyContent = 'center';
            traditionalContainer.style.alignContent = 'start';
            traditionalContainer.style.gap = '2px';
            traditionalContainer.style.gridTemplateColumns = isDouble ? 'repeat(2, 45vw)' : '90vw';

            for (let i = 1; i <= pdf.numPages; i++) {
              const page = await pdf.getPage(i);
              const viewport = page.getViewport({ scale: currentScale * 2 }); // 高分辨率渲染
              ...
              // 2) 显示尺寸（逻辑像素）：宽度用 vw，高度按 PDF 宽高比自适应
              const widthVw = isDouble ? 45 : 90;  // 双页=45vw/页，单页=90vw
              canvas.style.width  = `${widthVw}vw`;
              canvas.style.height = 'auto';
              // 现代浏览器支持 aspect-ratio，可让高度自动按比例计算
              canvas.style.aspectRatio = `${viewport.width} / ${viewport.height}`;
              // 若需兼容极老的浏览器，可在此处计算像素高度并设置 canvas.style.height（通常不需要）
            ...
            traditionalContainer.removeEventListener('scroll', debouncedScrollHandler);
            traditionalContainer.addEventListener('scroll', debouncedScrollHandler);
